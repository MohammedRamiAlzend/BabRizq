/**
 * Login page — Customer application.
 *
 * Single-role entry point: the customer app has no role picker. Entering the demo
 * credentials (username "1" / password "1") logs the user straight into the storefront.
 *
 * Real API contract simulated here:
 *   POST /api/auth/login  →  { token, role: 'customer', name, nameAr }
 */
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '@/features/auth/model/authContext';
import { useLocale } from '@/shared/contexts/LocaleContext';
import { ShoppingBag, Eye, EyeOff, LogIn, AlertCircle } from 'lucide-react';
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8 text-center lg:text-start">
            <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary mb-4">
              ...
              {t('Customer Storefront', 'واجهة المتجر')}
            </div>
            <h2 className="text-2xl font-bold text-foreground mb-1">
              {t('Sign in to start shopping', 'سجّل الدخول لبدء التسوق')}
            </h2>
