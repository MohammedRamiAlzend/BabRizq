/**
 * Login page — Marketer application.
 *
 * Single-role entry point: no role picker. Demo credentials (username "1" / password "1")
 * log the user straight into the marketer portal.
 *
 * Real API contract simulated here:
 *   POST /api/auth/login  →  { token, role: 'marketer', name, nameAr }
 */
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '@/features/auth/model/authContext';
import { useLocale } from '@/shared/contexts/LocaleContext';
import { Megaphone, Eye, EyeOff, LogIn, AlertCircle } from 'lucide-react';
import loginBg from '@/assets/login-bg.jpg';
import logo from '@/assets/logo.png';

const HOME_ROUTE = '/marketer';
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8 text-center lg:text-start">
            <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary mb-4">
              ...
              {t('Marketer Portal', 'بوابة المسوّق')}
            </div>
            <h2 className="text-2xl font-bold text-foreground mb-1">
              {t('Sign in to promote & earn', 'سجّل الدخول للترويج والربح')}
            </h2>
          ...
          <p className="mt-6 text-center text-xs text-muted-foreground">
            ...
            <span className="font-mono font-semibold"> marketer@babrizq.com </span>
