/**
 * Product entity — mock API (back office).
 *
 * Simulates the store-product stock endpoints from
 * `docs/needed-endpoints-from-backend.md`
 * (`GET /api/store-owner/products/{id}/stock`). Seed data is copied verbatim
 * from the legacy `entities/storeOwnerData.ts`.
 */
import { StoreProduct } from './model';

/** In-memory store catalogue. TODO(migration): replaced by the store-owner product API. */
...
/** Simulates `GET /api/store-owner/products/{id}/stock`. */
