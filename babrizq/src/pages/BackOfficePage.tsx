 ...
 * Headers: Authorization: Bearer <token>  (role must be "back_office")
...
import { ClipboardList, UserPlus, X, Search, Filter } from 'lucide-react';
...
const BackOfficePage = () => {
  ...
  if (!isAuthenticated || !user || user.role !== 'back_office') return <Navigate to="/" replace />;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container px-4 py-6 space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">{t(`${orders.length} total orders across all stores`, `${orders.length} طلب عبر جميع المتاجر`)}</p>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="text-start px-4 py-3 font-medium text-muted-foreground hidden md:table-cell">{t('Store', 'المتجر')}</th>
                  ...
                  <th className="text-start px-4 py-3 font-medium text-muted-foreground hidden lg:table-cell">{t('Driver', 'السائق')}</th>
                  <th className="text-end px-4 py-3 font-medium text-muted-foreground">{t('Action', 'إجراء')}</th>
                </tr>
              </thead>
                  ...
                  <tr key={order.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground sm:hidden">{t(order.customerNameEn, order.customerNameAr)}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden sm:table-cell text-foreground">{t(order.customerNameEn, order.customerNameAr)}</td>
                    <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{t(order.storeNameEn, order.storeNameAr)}</td>
                    <td className="px-4 py-3 text-foreground font-medium">{order.total} {t('SAR', 'ر.س')}</td>
      ...
      {assignModalOrderId && (
        <AssignDriverModal
          ...
          onAssign={(driverId) => { assignDriver(assignModalOrderId, driverId); setAssignModalOrderId(null); }}
...
const AssignDriverModal = ({ orderId, onAssign, onClose }: { orderId: string; onAssign: (driverId: string) => void; onClose: () => void }) => {
  const { t } = useLocale();
        ...
        <div className="w-full max-w-md rounded-2xl border border-border bg-card shadow-2xl" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <div>
              ...
              {order && <p className="text-xs text-muted-foreground mt-0.5">{order.orderNumber} — {t(order.customerNameEn, order.customerNameAr)}</p>}
            </div>
          ...
          <div className="px-6 py-5 space-y-4">
            <label className="block text-sm font-medium text-foreground mb-1.5">{t('Select Driver', 'اختر السائق')}</label>
            <div className="space-y-2">
              {MOCK_DRIVERS.map(driver => (
                ...
                >
                  ...
                  <span className={`text-[10px] font-medium rounded-full px-2 py-0.5 ${driver.available
